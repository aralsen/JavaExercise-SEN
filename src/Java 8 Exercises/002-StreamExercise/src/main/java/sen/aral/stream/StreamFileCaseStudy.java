...
//        Map<String, Integer> slaesMap = rowData.map(row -> row.split(","))
//                                                .filter(r->r.length>2)
//                                                .collect(Collectors.toMap(x->x[0], y->Integer.parseInt(y[1])));
//
...
//        System.out.println("Sales of Margherita: "+slaesMap.get("Margherita"));
        ...
        Optional<Integer> optTotalSales = rowData.map(x->x.split(","))
